...
void elasticity<dim>::assemble_system_interval (const typename std::vector<knotSpan<dim> >::iterator &begin, const typename std::vector<knotSpan<dim> >::iterator &end){
  ...
  for (typename std::vector<knotSpan<dim> >::iterator cell=begin; cell<end; cell++){
    ...
    getDeformationMapWithGradient<Sacado::Fad::DFad<double>, dim>(*fe_values, 0, ULocal, defMap, currentIteration);
    ...
    residualForMechanics(*fe_values, 0, FINITE_STRAIN_FLAG, ULocal, ULocalConv, R, currentIteration, localfreeEnergy, localinterfaceEnergy, localdirchletBC, *cell, numKnots, params, mesh, &derivedValueP);
...
//Adjust K and RHS to reflect BC's
template <int dim>
...
void elasticity<dim>::run (){
  ...
  double values[]={0.0, 0.01, 0.1, 1.0, 10.0};
